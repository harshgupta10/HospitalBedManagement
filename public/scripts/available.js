...
$(document).ready(function() {
  ...
  $.getJSON(patientsAPI).done(function(patients) {
	  ...
	  $.getJSON(roomsAPI).done(function(rooms1) {
          ...
          for(var room in rooms1) {
            var roomsRowConstructor = [];

          if (room !== 'noroom' ) {
                  roomsRowConstructor.push(room);
                  roomsTableConstructor.push(roomsRowConstructor);
		...
		//   });
            ...
            var roomsDashboard = roomsTableConstructor.length || 0;
            $("#rooms-live").html(roomsDashboard);
